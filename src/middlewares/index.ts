...
export const requireAccessTokenAdmin = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  // Verify token
  jwt.verify(token, config.secretKeyAdmin, (err, decoded) => {
    if (err) {
      errorResponse(res, "Invalid or expired access token", 403);
      return;
    }
    ...
    // Attach user info from token to request
    req.admin = decoded as IAdmin; // Ensure the decoded token is of type IUser
